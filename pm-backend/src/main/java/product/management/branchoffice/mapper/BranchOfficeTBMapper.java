...
public interface BranchOfficeTBMapper {
	...
	// Paging
	@Select("SELECT * FROM BRANCH_OFFICE_TB WHERE 1=1 AND USER_ID = #{user_id} AND BRANCH_OFFICE_NM LIKE CONCAT('%',#{keyword},'%') ORDER BY BRANCH_OFFICE_SQ DESC LIMIT #{cp}, #{ps}")
	...
	@Update("UPDATE BRANCH_OFFICE_TB SET BRANCH_OFFICE_NM = #{branch_office_nm}, BRANCH_OFFICE_AREA = #{branch_office_area}, MOD_DT = NOW() WHERE BRANCH_OFFICE_SQ = #{branch_office_sq}")
	int updateBranchOfficeTB(BranchOfficeTB branchOfficeTB);
	
	
	@Delete("DELETE FROM BRANCH_OFFICE_TB WHERE BRANCH_OFFICE_SQ = #{branch_office_sq}")
