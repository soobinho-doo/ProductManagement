<script lang="ts">
    ...
    import { isLogin } from "../option/auth";
    
</script>

<div class="down-menu-bar background-color-white border-top-b1 width-100 z-index-99">
    <div class="display-flex align-items height-50px">
        <!-- 메뉴 버튼 -->
        <button type="button" class="background-none border-none width-100" aria-label="menu" on:click={() => {push("/menu")}}>
        ...
        <!-- 검색 버튼 -->
        <button type="button" class="display-grid background-none border-none width-100" on:click={() => {push("/salesstatus")}}>
            <span class="fs-1rem pretendard-bold">판매</span>
            <span class="fs-1rem pretendard-bold">현황</span>
        </button>

        <!-- 홈 버튼 -->
        <button type="button" class="background-none border-none width-100" aria-label="home" on:click={() => {push("/")}}>
            <svg xmlns="http://www.w3.org/2000/svg" class="svg-color-change" width="32" height="32" viewBox="0 0 24 24">
        ...
        <!-- 로그인 버튼 -->
        {#if $isLogin}
            <button type="button" class="background-none border-none width-100" aria-label="mypage" on:click={() => {push("/mypage")}}>
        ...
        {:else}
            <button type="button" class="background-none border-none width-100" aria-label="login" on:click={() => {push("/login")}}>
                <svg xmlns="http://www.w3.org/2000/svg" class="svg-color-change" width="24" height="24" viewBox="0 0 448 512">
        ...
        <!-- 설정 버튼 -->
        <button type="button" class="background-none border-none width-100" aria-label="setting" on:click={() => {push("/setting")}}>
            <svg xmlns="http://www.w3.org/2000/svg" class="svg-color-change" width="32" height="32" viewBox="0 0 24 24">
